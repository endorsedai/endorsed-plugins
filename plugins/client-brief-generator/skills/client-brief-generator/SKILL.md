...
This skill requires the **Firecrawl MCP connector** to be configured in Claude.ai Cowork admin:

- Remote MCP URL: `https://mcp.firecrawl.dev/{FIRECRAWL_API_KEY}/v2/mcp`
- Auth: `Authorization: Bearer <API_KEY>` header
- Tools used: `firecrawl_map`, `firecrawl_scrape`, `firecrawl_crawl`

If Firecrawl is not wired up, ask the user to either:
...
Never fabricate website content. If neither Firecrawl nor a pasted file is available, stop and ask.
...
2. **Scrape the website via Firecrawl**
   - Use `firecrawl_map` to discover indexed URLs first
   - Then `firecrawl_scrape` on the key pages: homepage, about, product(s), pricing, case studies, customers, blog (latest 3-5 posts)
   - Save the combined markdown into the Project as `website-content.md` for future section refreshes
...
2. **Read the updated source material**
   ...
   - Website re-crawl? Use `firecrawl_scrape` on the changed pages only
...
│   ├── sales-deck.pdf
│   ├── pitch-deck.pdf
│   └── website-content.md       ← generated by Firecrawl during full brief
├── 01-generated/
